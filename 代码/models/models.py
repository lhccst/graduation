...
from models.trainmodel.bilstm import *
from models.trainmodel.resnet import *
from models.trainmodel.alexnet import *
from models.trainmodel.mobilenet_v2 import *
from models.trainmodel.transformer import *
...
class BaseHeadSplit(nn.Module):
    def __init__(self, args, cid, feature_dim=None):
        ...
        # self.projection = nn.Linear(feature_dim, feature_dim)  # 投影层，统一输出为128维
        # self.projection = copy.deepcopy(args.projection)
        if self.base.__class__.__name__ == "ResNet":
            if args.in_channels == 1:  # 如果输入是灰度图像
        ...
        if self.base.__class__.__name__ == "MobileNetV2":
            if args.in_channels == 1:  # 如果输入是灰度图像
        ...
        if self.base.__class__.__name__ == "GoogLeNet":
            if args.in_channels == 1:  # 如果输入是灰度图像
                self.base.conv1.conv = nn.Conv2d(
                    in_channels=1,  # 修改输入通道数为1
                    out_channels=self.base.conv1.conv.out_channels,  # 获取实际的输出通道数
                    kernel_size=self.base.conv1.conv.kernel_size,  # 保留原始的卷积核大小
                    stride=self.base.conv1.conv.stride,  # 保留原始的步长
                    padding=self.base.conv1.conv.padding,  # 保留原始的填充方式
                    bias=self.base.conv1.conv.bias is not None  # 保留是否使用偏置
                )
        ...
        if hasattr(args, 'heads'):
            ...
            # self.head = nn.Sequential(
            #     nn.Linear(feature_dim, feature_dim),  # 第一层 Linear，输入特征维度到 512
            #     nn.ReLU(),  # 激活函数
            #     nn.Linear(feature_dim, args.num_classes)  # 第二层 Linear，从 512 到类别数
            # )
