...
class SupConLoss(nn.Module):
    ...
    def forward(self, features, labels=None, mask=None):
        ...
        beta = 0.1
        sim_mat_exp = torch.exp(-beta * sim_mat_noDIV) ##dissim
        # ##欧几里得
        # cost_mat = self.EuclideanDistances(anchor_feature, contrast_feature.T)
...
class LabeledGlobalConLoss(nn.Module):
    ...
    #         raise ValueError('Unknown mode: {}'.format(self.contrast_mode))
    #
    #     global_protos_dict = dict(global_protos)
    #     # generate anchor_feature
    #     for i in range(batch_size * anchor_count):
    #         anchor_feature[i, :] = global_protos_dict.get(labels[i % batch_size].item())
    #
    #     # compute logits
    #     anchor_dot_contrast = torch.div(
    #         torch.matmul(anchor_feature, contrast_feature.T),
