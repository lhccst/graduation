...
from src.pFedAFM.client_pFedAFM import ClientpFedAFM

class ServerpFedAFM(ServerBase):
    def __init__(self, args):
        super().__init__(args)
        # self.shared_params = copy.deepcopy(args.head.state_dict())
        # self.aggregatedHead =  copy.deepcopy(args.head.state_dict())
        self.aggregated = copy.deepcopy(BaseHeadSplit(args, 0)).state_dict()
        self.initialize_clients(ClientpFedAFM)
        self.class_distribution = defaultdict(list)

    def dispatch(self):
        for client in self.join_clients:
            self.reporter.track_download(self.aggregated)
            client.net_small.load_state_dict(self.aggregated)

    def receive_and_aggregated(self):
        client_models = []
        client_sample_sizes = []
        for client in self.join_clients:
            client_models.append(client.net_small.state_dict())
            client_sample_sizes.append(client.sample_size)
            self.reporter.track_upload(client.net_small.state_dict())
