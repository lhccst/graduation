...
def save_result(dataset, distribution, algorithm, num_clients, result, prefix,debug):
    if debug:
        result_dir = f"./results/{dataset}/{distribution}"
    else:
        result_dir = f"./finalResults/{dataset}/{distribution}"
    ...
    if algorithm=='FedCPCL':
        ...
        result_path = os.path.join(result_dir, file_prefix + f"{algorithm}-{num_clients}-{alpha}-{lambada}.json")
        print("saving in: ", result_path)
    else:
